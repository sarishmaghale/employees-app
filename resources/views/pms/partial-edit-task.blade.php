                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            ...
                            <textarea class="form-control mb-3" rows="2" placeholder="Write a comment..."></textarea>
                            <div>
                                <div class="d-flex align-items-start mb-2">
                                    <div class="rounded-circle bg-warning text-center me-2"
                                        style="width:32px; height:32px; line-height:32px;">Y</div>
                                    <div>
                                        <strong>yaammmii</strong> completed one on this card<br>
                                        <small class="text-muted">26 Dec 2025, 13:28</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        modal.find('#pmsEditTaskTitle').html(
                            `<i class="far fa-circle"></i> ${response.data.title}`
                        );
                        modal.find('#pmsEditTaskDesc').val(response.data.description ?? "");
                        modal.find('#pmsEditTaskStart').val(response.data.start_date ?? "");
                        modal.find('#pmsEditTaskEnd').val(response.data.end_date ?? "");
                    } else {
